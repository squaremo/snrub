    ...
    if (that.prefix_test.test(req.url)) {
      ...
      if (req.method === 'GET') {
        ...
        if (!(mode && topic && challenge && token)) {
          return respond(res, 404, "Invalid verification request");
        }
        ...
        if (memo) {
          ...
          if (mode === 'subscribe') {
            ...
            if (memo.expiry && memo.expiry < +new Date) {
              return respond(res, 404, "Expired subscription");
            }
              ...
              return respond(res, 404, "Invalid hub.verify_token");
            }
          }
          else if (mode === 'unsubscribe') {
            if (that.token.lookup(token, mode, topic)) {
            ...
            return respond(res, 404, "Invalid hub.mode");
          }
        }
        else {
...
proto.subscribe = function(hubUrl, topic, opts, callback, errback) {
  ...
  if (opts.leaseSeconds) { qsopj['lease_seconds'] = opts.leaseSeconds; }
...
function subscribeRequest(hubUrl, qsobj, path, callback, errback) {
  ...
  req.end(querystring.stringify(qsobj), 'utf8');
...
exports.cryptoPathProvider = function(secret) {
  return {
    'create': function(sub) {return '/' + encryptJson(sub, secret);},
    'lookup': function(path) {
      if (path.charAt(0) == '/') {
        return decryptJson(path.substr(1), secret);
      }
      return false;
    }
  };

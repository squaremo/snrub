  ...
  proto.newEntries = function(topic, entries) {
    ...
    entries.forEach(function(entry) {
      ...
      if (!topicSet.includes(entry.id)) {
        topicSet.add(entry.id, entry.timestamp);
        newer.push(entry);
      }
    });
...
(function(proto) {
  ...
  proto.add = function(datum, score) {
    this._set[datum] = new Item(datum, score);
    return this;
  }

  proto.includes = function(datum) {
    return this._set[datum];
  }
